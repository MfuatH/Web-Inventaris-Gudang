<x-app-layout>
                    ...
                    <table class="min-w-full bg-white">
                        <thead class="bg-gray-800 text-white">
                            <tr>
                                ...
                                <th class="text-left py-3 px-4 uppercase font-semibold text-sm">Jadwal Selesai</th>
                            ...
                            <tr>
                                ...
                                <td class="py-3 px-4">
                                    @if($request->link_zoom)
                                        <a href="{{ $request->link_zoom }}" target="_blank" class="text-blue-600 hover:underline">Link Zoom</a>
                                    @else
                                        <button onclick="openLinkModal({{ $request->id }})" class="text-blue-500 hover:text-blue-700">Masukkan Link</button>
                                    @endif
                                </td>
                                <td class="py-3 px-4">
                                    @if($request->link_zoom)
                                    ...
                                    @else
                                    <span class="text-gray-500">Masukkan link terlebih dahulu</span>
                                    @endif
                                </td>
                            </tr>
                            @empty
                            <tr>
                                <td colspan="8" class="text-center py-4">Tidak ada permintaan Link Zoom yang masuk.</td>
                            </tr>
                            @endforelse
                        </tbody>
                    </table>
    ...
    <script>
        function openLinkModal(requestId) {
            ...
            document.getElementById('linkForm').action = `/zoom/${requestId}/add-link`;
        }
